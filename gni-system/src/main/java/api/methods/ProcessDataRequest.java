...
public class ProcessDataRequest {
    ...
    public static void handleDataRequestExceptions(
            final DataRequest dataRequest, final String cookie, final ApiBean api) {
        try {
            verifyDataRequestInput(dataRequest);
            doDataRequest(dataRequest, cookie, api);
     ...
     */
    private static void doDataRequest(final DataRequest dataRequest, final String cookie, final ApiBean api) {
                ...
                (httpStatusCode, httpContentType, dataReplyJson) -> {
                    if (httpStatusCode == HTTP_OK) {
                        ...
                        if (!messageWrapper.isError()) {
                            processDataReply((DataReply) messageWrapper.getData(), dataRequest, api);
                        } else {
     ...
     */
    private static void processDataReply(final DataReply dataReply, final DataRequest dataRequest, final ApiBean api) {
        RequestType requestType = dataRequest.getType();
        switch (requestType) {
            case BALANCE:
                ...
                result.put("balance", dataReply.getAccountData().getBalance());
